...
def autocomplete():
    user_input = request.args.get('input') # FIX: The parameter name is 'input'
    if not user_input:
        return jsonify([])

    # FIX: This now calls the actual Google Places API
...
def get_route():
    try:
        # FIX: Using the correct parameter names that the app is sending
